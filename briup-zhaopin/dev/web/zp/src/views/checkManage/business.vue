 ...
 * @Last Modified time: 2019-12-29 20:37:46
 */
<template>
  <div id="businessList">
    <div class="tableDiv">
      <div class="selectDiv">
         <el-select @change="statusChange" size="small" v-model="status" clearable placeholder="全部">
      ...
      >
        ...
        <el-table-column align="center" label="详情">
          <template slot-scope="scope">
            <el-button @click="toSee(scope.row)" type="text" size="small" icon="el-icon-view">查看</el-button>
          </template>
        </el-table-column>
        <!-- 待审核状态操作 -->
    ...
    <div class="footerDiv">
      ...
      <!-- 一键审核通过 -->
      <div class="btnDiv">
        <el-button @click="toadopt()" type="danger" plain size="small">一键通过</el-button>
      </div>
    </div>
    <!-- 查看详细数据按钮  -->
    ...
    <el-dialog title="提示" :visible.sync="adoptVisible" style="width:40%">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="adoptVisible = false" size="small">取消</el-button>
    ...
    >
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm">
        <el-form-item label="拒绝理由" prop="name">
          ...
          <el-button @click="resetForm('ruleForm')" class="Reset" type="text" size="small">重置</el-button>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click="refuse('ruleForm')" size="small">取 消</el-button>
        <el-button type="primary" @click="torefuse('ruleForm')" size="small">确 定</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    async statusChange(a) {
      let all = await findAllBusiness();
      if (a) {
        this.businessData = all.data.filter(item => {
          return item.status === a;
        });
      } else {
        this.businessData = [...all.data];
      }
    },
    handleClose() {
